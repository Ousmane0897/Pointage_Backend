...
- **E-mail à la création d'un bon** (`BonMailNotificationService`, 2026-07-29) : `POST /api/stock/bons-entree` et `/bons-sortie` envoient un mail HTML récapitulatif (référence, type, date, site, destinataire/fournisseur, nb de lignes, montant, auteur) + lien vers la fiche. **Destinataires** = le `SUPERADMIN` (collection `login`, seul endroit où ce rôle existe → `UserRepository.findByRoleIgnoreCase`) et les `CONTROLEUR_STOCK` actifs (collection `utilisateur` → `UtilisateurRepository.findByRoleAndActiveTrue`), sur leur **adresse de connexion**, dédoublonnée, **un envoi par destinataire**. Réutilise `EmailService.sendHtmlEmail` (même mécanique que le mail « mot de passe oublié ») en **synchrone** — `@EnableAsync` n'existe pas dans le projet. Lien construit depuis `app.frontend.base-url` = **URL publique de l'application** (ce que l'utilisateur ouvre dans son navigateur, à ne pas confondre avec l'API) : `http://localhost:4200` en dev, `https://pointic-cleanic.com` en prod — aligné sur le CORS de `SecurityConfig` —, surchargeable par `APP_FRONTEND_BASE_URL` ; si vide, le mail part sans lien. ⚠️ **Tolérance aux pannes obligatoire** : tout échec (SMTP, Mongo) est capté et loggé en `WARN` — la création du bon doit aboutir même sans mail. Aucun envoi sur `modifier`/`soumettre`/`valider`/`refuser`.
